...
class TradesemGitHubSupabaseUser extends BaseAuthUser {
  TradesemGitHubSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> tradesemGitHubSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TradesemGitHubSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
